...
def init_weights(m):
    if isinstance(m, nn.Linear) or isinstance(m, nn.Conv2d):
        torch.nn.init.xavier_uniform_(m.weight)  # 仅初始化 2D 以上的权重
        if m.bias is not None:
            torch.nn.init.zeros_(m.bias)  # 偏置初始化为 0
    elif isinstance(m, nn.Embedding):
        torch.nn.init.uniform_(m.weight, -0.05, 0.05)  # 适用于 Embedding
    elif isinstance(m, nn.BatchNorm2d):
        torch.nn.init.ones_(m.weight)  # BN 层 weight 初始化为 1
        torch.nn.init.zeros_(m.bias)   # BN 层 bias 初始化为 0
...
if os.path.exists("pacman.pth"):
    policy_net_pacman.load_state_dict(torch.load("pacman.pth",map_location=device, weights_only=True))
else:
    print("No checkpoint found. Training from scratch.")
    policy_net_pacman.apply(init_weights)
...
if os.path.exists("ghost.pth"):
    policy_net_ghost.load_state_dict(torch.load("ghost.pth",map_location=device, weights_only=True))
else:
    print("No checkpoint found. Training from scratch.")
    policy_net_ghost.apply(init_weights)
...
if __name__ == "__main__":
    ...
    for episode in range(num_episodes):
        ...
        for t in range(1000):
            ...
            reward1 = torch.tensor(np.array([reward1]), dtype=torch.float32)
            reward2 = torch.tensor(np.array([reward2]), dtype=torch.float32)
            ...
            memory.append(
                Transition(
                    state,
                    extra,
                    torch.tensor(np.array([[action1]])),
                    torch.tensor(np.array([[action2]])),
                    next_state,
                    next_extra,
                    reward1,
                    reward2,
        ...
        if episode % TARGET_UPDATE == 0:
            print("(Iteration %d / %d) loss: %f,%f"% (episode, num_episodes, Loss1[-1],Loss2[-1]))
            target_net_pacman.load_state_dict(policy_net_pacman.state_dict())
            target_net_ghost.load_state_dict(policy_net_ghost.state_dict())
